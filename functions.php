...
// Hook into admin_post to process the button click for logged-in users.
add_action('admin_post_update_floorplans_status', 'update_floorplans_status_to_sold_out');

function update_floorplans_status_to_sold_out() {
    ...
    if (!$property_id) {
        wp_die('Invalid property ID.');
    }

    // Query floorplans linked to the specified property via the associated_property taxonomy
    $args = [
    ...
    wp_redirect(admin_url('admin.php?page=your_custom_page_slug&success=floorplans_updated'));
    exit;
}
