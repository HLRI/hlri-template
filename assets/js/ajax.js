jQuery(document).ready(function ($) {
  $("#latest-tab").html(
    '<div class="text-center"> <div class="spinner-border spinner-border-sm text-warning" role="status"></div></div>',
  );
  $.ajax({
    type: "post",
...
jQuery("#submit-register").on("click", function (e) {
  ...
  jQuery(".register-form .notif-info")
    .removeClass("success-info")
    .addClass("d-none");
  jQuery(".register-form .notif-info")
    .removeClass("error-info")
...
jQuery("#submit-forgot-password").on("click", function (e) {
  ...
  jQuery(".forgot-password-form .notif-info")
    .removeClass("success-info")
    .addClass("d-none");
  jQuery(".forgot-password-form .notif-info")
    .removeClass("error-info")
        ...
        success: function (data) {
          ...
          if (data_mapbox.features != "") {
            jQuery.each(data_mapbox.features, function (i, item) {
              jQuery(".search-result").prepend(
                '<div class="result-card mt-1 mb-2 px-3"><a href="#">' +
                  item.place_name +
                  "</a></div>",
              );
            });
